...
export default async function GrupoIdPage( { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from('groups').select(`name, participants (*)`).eq('id', groupId).single();

  if (data === null) {
    return <p>Sem grupos ainda. Crie novos grupos de amigo secreto.</p>
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-2xl">
              Grupo
              <span className="font-light underline">{data.name}</span>
          </CardTitle>
        </div>
        <CardDescription>
